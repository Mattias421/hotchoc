...
class SOrthConv(nn.Module):
    ...
    def reset_parameters(self):
        ...
        nn.init._no_grad_normal_(
            self.conv.weight, 0.0, self.get_M_shape(self.conv.weight)[1] ** -0.5
        )
...
class FTDNNLayer(nn.Module):
    def __init__(
        ...
        # Adjust input dimension if using time embedding
        factor1_input_dim = in_dim
        if self.use_time_embedding:
            factor1_input_dim += self.time_embedding_dim
    ...
    def forward(self, x, t=None):
        ...
        assert x.shape[-1] == (
            self.factor1.conv.weight.shape[1]
            - (self.time_embedding_dim if self.use_time_embedding else 0)
        ...
        if self.use_time_embedding:
            assert t is not None, (
                "Time embedding must be provided when use_time_embedding=True"
            )
            # Expand time embedding to match sequence length
            t_emb = t.unsqueeze(1).expand(-1, x.shape[1], -1)
            # Concatenate time embedding with input features
...
class TDNN(nn.Module):
    def __init__(
        ...
        # Adjust input dimension if using time embedding
        conv_input_dim = self.input_dim
        if self.use_time_embedding:
            conv_input_dim += self.time_embedding_dim
        ...
        # Time embedding MLP (following standard diffusion model practices)
        if self.use_time_embedding:
...
class FTDNN(nn.Module):
    def __init__(self, in_dim=30, use_time_embedding=False, time_embedding_dim=128):
        """
    ...
    def forward(self, x, t=None):
        ...
        x_3 = self.layer03(x_2, t)
        x_4 = self.layer04(x_3, t)
        skip_5 = torch.cat([x_4, x_3], dim=-1)
        x = self.layer05(skip_5, t)
        x_6 = self.layer06(x, t)
        skip_7 = torch.cat([x_6, x_4, x_2], dim=-1)
        x = self.layer07(skip_7, t)
...
def sinusoidal_embedding(timesteps, embedding_dim, max_period=10000):
    """
    Create sinusoidal timestep embeddings.
    Same as in the original DDPM paper: https://arxiv.org/abs/2006.11239
    """
